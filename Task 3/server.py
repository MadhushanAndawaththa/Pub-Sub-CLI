...
class ClientThread(threading.Thread):
    ...
    def run(self):
        print("{}{}{} connected from : {}".format((self.mode).lower(), self.topic, self.id, self.addr))
    ...
    def handle_publisher(self):
        ...
        while True:
            ...
            # Send the message to all subscriber clients on specific topics
            for client in clients:
                if client.mode == "SUBSCRIBER" and client.topic == self.topic:
                    message = "From publisher" + str(self.id) + " on Topic " + self.topic + " : " + data
...
def run_server(port):
    ...
    while isActive.is_set():
        ...
        if server_socket in rlist:
            ...
            decoded = client_socket.recv(1024).decode()
            mode = decoded.split()[0]
            topic = decoded.split()[1]
